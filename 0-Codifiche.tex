...
\textbf{Esempio:} \\
Se l'input è un array $x = (4, 79, 149)$, la dimensione dell'input è $n = |x| = 3$. \\
...
%Se la codifica dell'input é ragionevole, ogni macchina di Turing deve almeno avere il tempo di scandirlo. Di conseguenza, il tempo $f(n)$ é sempre limitato inferiormente da $n$: \\
%\boxed{f(n) = \Omega(n)}

%Inoltre, se sappiamo che una macchina di Turing termina in tempo $f(n)$, possiamo esser certi che lo spazio utilizzato $g(n)$ sia limitato superiormente dal tempo. \\
%\boxed{g(n) = O(f(n))}
