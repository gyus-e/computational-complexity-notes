...
Poiché sappiamo che $f(n)$ é limitato inferiormente da $n$ (teroema \ref{teorema:tempo_limitato_inferiormente_dall_input}), possiamo dire che il costo della copia dell'input é $O(f(n))$. \\
Il costo della simulazione di M é $O(f(n))$. \\
Poiché sappiamo che lo spazio é limitato superiormente da $f(n)$ (teorema \ref{teorema:spazio_limitato_superiormente_dal_tempo}), il costo della copia dell'output sull'ultimo nastro é $O(f(n))$, . \\
...
$M_f$ Scrive $\sqcap^{f(n)}$ sul nastro $k$  impiegando sempre tempo $O(n + f(n)) = O(f(n))$. (teorema \ref{teorema:tempo_limitato_inferiormente_dall_input})\\
...
\input{teoremi/Speedup_multinastro}


\input{teoremi/Linear_speedup}