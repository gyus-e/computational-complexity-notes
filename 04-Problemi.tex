...
\begin{itemize}
    ...
    \item $\forall v \in V \smallsetminus \{s, t\} : \sum_{(u, v) \in E} f(u, v) = \sum_{(v, w) \in E} f(v, w)$. 
...
\subsection{$\SAT$}
\label{problema:sat}
Se $PL$ é l'insieme di tutte le formule della logica proposizionale, 
e $AP(\varphi)$ é l'insieme di tutte le proposizioni atomiche contenute 
in una formula $\varphi \in PL$:
\[
\SAT = \{ \varphi \in PL \mid \exists M \subseteq AP(\varphi) : M \models \varphi \} 
\]
\[
n = |\varphi|
\]

É un problema che richiede di esplorare uno spazio esponenziale di possibili assegnazioni a ogni proporizione atomica in $\varphi$
(che é esattamente $2^{|\varphi|}$)\\
Un problema affine é quello del \textbf{model checking}: dato un modello $M$, verifica se $M \models \varphi$.
...
Basterebbe infatti rappresentare $\varphi$ tramite l'albero sintattico ed effettuare una visita in post-order,
...
senza preoccuparsi di cercarne uno.
...
Questo problema puó essere risolto con l'algoritmo di risoluzione, eliminando proposizioni in clausole diverse che vanno in 
\textit{clash} tra loro. \\
Lo spazio da esplorare é comunque esponenziale sul numero di clausole (bisogna esplorare tutte le possibili combinazioni di proposizioni da eliminare). \\
