...
\[
\REACHABILITY = \{ (V, E, s, t) \mid s \text{ raggiunge } t \text{ in } G \} 
\]
\[
n = |V| + |E| + 2
\]
...
\begin{itemize}
	...
	\begin{itemize}
        \item chiamiamo $c_{max} = \max_{(u, v) \in E} c(u, v)$
        \item supponiamo che i valori di $c$ siano rappresentati in binario. $|c| = |E| \cdot \log_2(c_{max})$
    \end{itemize}
...
$\log_2(m) = O(\log_2(|E| \cdot c_{max})) = O(\log_2(2^n)) = O(n)$. \\
...
\[
\TSP: (V, E, w) \in \Sigma^* \rightarrow (\pi : V \rightarrow \{1,\dots,|V|\}) \in {\{1,\dots,|V|\}}^V \text{, con } \pi \text{ biettiva}
\]
\[
n = |V| + |E| + |w|
\]
...
\[
\TSP_D = \{ (V, E, w, B) \mid G \text{ ha un ciclo hamiltoniano di costo } \leq B \}
\]
\[
n = |V| + |E| + |w| + \log_2(B)
\]
