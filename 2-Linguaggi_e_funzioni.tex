...
\[\MAXFLOW: x \in \Sigma^* \rightarrow (f: E \rightarrow \mathbb{N}) \in \mathbb{N}^E\]
...
\[
\MAXFLOW_D = \{ (G, k) \mid G \text{ ha un flusso di valore } \geq k \}
\]
Si inizia prendendo un flusso $f(u, v) = 0$ per ognu arco $(u, v) \in E$. \\
Verifichiamo se é possibile aumentare il flusso trovando un cammino di miglioramento da $s$ a $t$ 
che permette di aumentarlo. L'aumento é pari all'arco di costo minimo lungo il cammino. \\
Consideriamo ora se ci sono archi $(u, v)$ lungo il cammino di miglioramento che hanno giá flusso massimo, cioé $f(u, v) = c(u, v)$. \\
In questo caso, possiamo invertire l'arco $(u, v)$, aggiungendo l'arco $(v, u)$ con capacitá $c(v, u) = f(u, v)$,
Se il cammino di miglioramento passa per $(v, u)$, allora possiamo diminuire il flusso su $(u, v)$ di $f(u, v)$. \\
L'algoritmo termina quando non é piú possibile trovare un cammino di miglioramento.


\subsubsection{Da $\MAXFLOW_D$ a $\MAXFLOW$}
Possiamo usare $\MAXFLOW_D$ per risolvere $\MAXFLOW$ tramite una ricerca sul valore del flusso massimo. \\
Cerchiamo quindi $k$ tale che $\MAXFLOW_D(G, k) = "yes"$ e $\MAXFLOW_D(G, k+1) = "no"$. \\
Il valore piú alto possibile di $k$ é determinato dal minimo tra la somma delle capacitá degli archi uscenti da $s$ e la somma delle capacitá degli archi entranti in $t$:
\[C = \min{(\quad \sum_{v \in Adj[s]} c(s, v), \sum_{u \in Adj[t]} c(u, t) \quad)}\]
Notare che questo valore massimo é esponenziale rispetto alla dimensione dell'input, poiché le capacitá degli archi sono rappresentate in binario. \\
Si puó effettuare una ricerca binaria tra $0$ e il valore massimo, interrogando $\MAXFLOW_D$ per ogni valore di $k$.
...
\[\TSP: x \in \Sigma^* \rightarrow \pi \in \{1,\dots,|V|\}^{\{1,\dots,|V|\}}\]

\subsubsection{$\TSP_D$}
Dati $G = (V, E, w)$ e un numero naturale $B \in \mathbb{N}$ detto \textit{budget}:
\[
\TSP_D = \{ (G, B) \mid G \text{ ha un ciclo hamiltoniano di costo } \leq B \}
\]
