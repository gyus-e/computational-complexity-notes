...
\[H_f = \{\underline{M}, x \mid M \text{ accetta } x \text{ entro } f(n) \text{ passi} \} \]
...
ma aggiungendone uno per separare $x$ da $\underline{M}$ e un altro per avere un contatore che garantisca la terminazione in $f(n)$ passi. \\

$f$ é propria: iniziamo ad applicare $M_f$ (sezione \ref{sec:funzioni_proprie}) per scrivere $\sqcap^{f(n)}$ su un nastro, 
in modo da utilizzarlo come contatore del numero di passi durante la simulazione di $M$. \\
Tempo: $O(n + f(n)) = O(f(n)) = O(f(N))$. \\
...
La lunghezza del nastro su cui legge la configurazione di $M$ é $k \cdot \ell \cdot O(f(n))$.
Di conseguenza, il tempo per leggere la configurazione é $O(f(n)) = O(f(N))$.\\
...
In questo modo la terminazione é garantita entro $O(f(N))$ transizioni.\\
